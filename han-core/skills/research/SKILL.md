...
- **Readability is applied while writing, held to the default audience frame.** The skill loads `../../references/readability-rule.md` and applies it as it writes the report, holding the default audience frame: a capable reader who did not do this work and lacks the author's context. It operates on prose regions only, so code fences, diagram bodies, and the `A#`/`V#` citation identifiers survive unchanged and every cited `A#` still resolves.
...
**Readability self-check.** Run the standardized readability self-check from `../../references/readability-rule.md` over the report's prose regions only — never inside code fences, diagram bodies, or citation identifiers (`A#`/`V#` survive unchanged). Confirm each criterion and fix any failure before presenting:
