...
> **Short answer.** Install the bundled suite with `/plugin install han@han`. That gives you everything the meta-plugin bundles: the planning, research, analysis, and documentation skills, every agent, the coding skills (writing, reviewing, analyzing, testing, investigating, and standardizing code), the GitHub skills, and the reporting skills. Pick `han.core` instead only when you know you do not want the coding, GitHub, or reporting skills. There is no coding-only, GitHub-only, or reporting-only option, because those plugins depend on the core plugin and bring it along. Some plugins sit outside the bundle: install `han.feedback` separately if you want to send feedback, `han.atlassian` separately if you want to publish documentation or feature plans to Confluence, or work items to Jira, and `han.plugin-builder` separately if you want the guidance for building your own skills, agents, and plugins.
...
- **`han.core`.** The heart of the suite. It carries the planning, research, analysis, and documentation skills, plus every agent the skills dispatch. If you install only this, you have the full set of specialists, but not the coding skills: `/code-review`, `/architectural-analysis`, `/investigate`, `/test-planning`, and `/coding-standard` now ship in `han.coding`. See the [Skills Index](./skills/README.md) for the complete list.
...
`han.coding` carries only the coding skills, `han.github` only the GitHub skills, and `han.reporting` only the reporting skills, so you might expect installing one to give you that slice of Han on its own. None of them work that way.
...
- **Core only** (`han.core`): the planning, research, analysis, and documentation skills, plus every agent. No coding, GitHub, or reporting skills, so no `/tdd`, `/code-review`, `/architectural-analysis`, `/investigate`, `/test-planning`, or `/coding-standard`.
- **The bundled suite** (`han`): all of the above, plus the coding (`/tdd`, `/code-review`, `/investigate`, and the rest), GitHub, and reporting skills.
