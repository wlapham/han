...
- **`han.coding`.** The code-writing layer: the suite's execution skills, the ones that change your source tree. It adds [`tdd`](./skills/han.coding/tdd.md), which drives a feature or behavior through a BDD-framed red-green-refactor loop and writes the tests and production code into your tree, and [`refactor`](./skills/han.coding/refactor.md), which restructures existing code without changing its behavior through a test-gated refactoring loop. This plugin depends on `han.core` and is bundled by the `han` meta-plugin, so the bundled suite includes it; a core-only install does not.
