...
- **`han.atlassian`.** The Atlassian layer. It adds [`confluence-project-documentation`](./skills/confluence-project-documentation.md), which runs the core documentation skill and then publishes the result to a Confluence location you specify, and [`work-items-to-jira`](./skills/work-items-to-jira.md), which creates one Jira ticket per slice from a work-items file. Both work through the Atlassian MCP server. This plugin depends on `han.core`, but it is **opt-in**: the `han` meta-plugin does not pull it in, so you install it on its own. It also requires a configured Atlassian MCP server.
