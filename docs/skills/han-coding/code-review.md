...
- **Documentation compliance and freshness.** ADRs, coding standards, and general docs are read and checked against the diff. Only documents whose subject matter the change actually touches are read — pulling unrelated standards into the review dilutes the signal and degrades judgment, the same reason Step 1.5 caps branch context — and correctness- and behavior-bearing rules are weighted over style minutiae the linter already enforces. Stale docs that misdescribe current behavior become CRIT findings.
