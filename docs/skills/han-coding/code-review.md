...
- **Documentation compliance and freshness.** ADRs, coding standards, and general docs are read and checked against the diff. Only documents whose subject matter the change touches are read; pulling unrelated standards into the review dilutes the signal and degrades judgment, the same reason Step 1.5 caps branch context. Correctness- and behavior-bearing rules are weighted over style minutiae the linter already enforces. Stale docs that misdescribe current behavior become CRIT findings.
...
- **Plan review.** Use [`/iterative-plan-review`](../han-planning/iterative-plan-review.md) for reviewing a work plan, not code.
...
- **Use focus hints for high-stakes branches.** When a branch touches a load-bearing surface (auth, billing, data migrations), name it in the prompt. The skill biases manual attention toward the area while the parallel agents still cover the full scope.
