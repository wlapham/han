...
- **Specialists are signal-selected.** `concurrency-analyst` joins when the code uses concurrency primitives. `adversarial-security-analyst`, `data-engineer`, and `devops-engineer` join when the focus area touches auth/PII, schemas/data contracts, or operational surface. `codebase-explorer` joins for large, unfamiliar areas. `system-architect` joins at large size when the focus area crosses a service or bounded-context seam. An agent whose domain the code does not touch is not dispatched, because that only burns tokens and dilutes the report.
- **The roster scales with size.** Small runs the spine plus concurrency. Medium adds one or two of the security, data, and DevOps specialists. Large adds the rest, the codebase map, and the system architect when a cross-service seam is present. The skill defaults to small and announces the chosen size and roster, with a one-line justification, before dispatching.
- **Numbered findings.** Each analyst returns findings with its own prefix: `S#` structural, `B#` behavioral, `C#` concurrency, `SEC-###` security, `DOR-###` DevOps, `R#` risk, `A#` software-architecture, `SA#` system-architecture. Cross-references survive into the recommendations so every proposed change traces to the finding that drove it.
