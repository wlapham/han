...
- **What it does.** Takes a `work-items.md` file produced by [`/plan-work-items`](./plan-work-items.md), validates the format, and creates one Linear issue per work item in a single target team through the Linear MCP server.
...
- **Posting to Jira.** Use [`/work-items-to-jira`](./work-items-to-jira.md) to create Jira tickets instead.
...
- [`/work-items-to-jira`](./work-items-to-jira.md). The Jira sibling that creates tickets instead of Linear issues.
- [`/work-items-to-issues`](./work-items-to-issues.md). The GitHub sibling that creates issues instead of Linear issues.
