...
- **[`/code-overview`](./han-coding/code-overview.md).** Produce a human-readable, progressive-disclosure overview of unfamiliar code (code mode) or a PR's changes (PR mode) — what it does, how it flows, and where to start — written to a scratch file, at minimal technical detail. Understand-now orientation, not durable docs and not a quality review. Dispatches `codebase-explorer` scaled with [size](../sizing.md); raises no findings.
