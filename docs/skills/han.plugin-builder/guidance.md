...
- **Run `init` once per repo, `update` after plugin upgrades.** Initialization is the first install; Update is the refresh. Update checks that both the vendored `.claude/skills/guidance/` directory and the rule index exist before touching anything, and offers to initialize if they do not.
